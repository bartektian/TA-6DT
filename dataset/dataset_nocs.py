...
class FarthestSampler:
    ...
    def sample(self, pts, k):
        ...
        farthest_pts[0] = np.mean(pts, axis=0)
    ...
    def sampledst(self, pts, k, node1):
        farthest_pts = np.zeros((k, 3))
        farthest_pts[0] = node1
        distances = self.calc_distances(farthest_pts[0], pts)
        for i in range(1, k):
            farthest_pts[i] = pts[np.argmax(distances)]
            distances = np.minimum(distances, self.calc_distances(farthest_pts[i], pts))
...
class Dataset(data.Dataset):
    ...
    def get_frame(self, choose_frame, choose_obj, syn_or_real):
        ...
        pc_radius_mask_np = pc_xz_norm_np <= np.partition(pc_xz_norm_np, -400)[::-1][400 - 1]
        ...
        if syn_or_real:
            return img, choose, cloud, target_r, target_t, target, mesh_pts, mesh_bbox, mask_target
        else:
            return img, choose, cloud, target_r, target_t, target, mask_target
    ...
    def caculate_diagonal(self, point_cloud):
        distances = np.sqrt(np.sum((point_cloud[:, np.newaxis, :] - point_cloud[np.newaxis, :, :]) ** 2, axis=-1))
    ...
    def __getitem__(self, index):
        ...
        if syn_or_real:
            ...
            while 1:
                try:
                    ...
                    index = random.randint(0, len(self.real_obj_list[self.cate_id][choose_obj]) - 2)
                    choose_frame = self.real_obj_list[self.cate_id][choose_obj][index:index + 2]
        ...
        return self.norm(torch.from_numpy(img_fr.astype(np.float32))), \
            ...
            torch.from_numpy(delta_RT.astype(np.float32))
